...
require 'rbender/mongo'
require 'rbender/processor'
...
module RBender
  module Processor
    class Base
      include RBender::Support::SessionManager
      ...
      def run!
        ...
        Telegram::Bot::Client.run(@token) do |bot|
          ...
          instance_exec(@api, @mongo_client, &@modules_block) if @modules_block
      ...
      def configure!(params)
        RBender::Mongo.setup params['title'], params['mongo']
        session_setup RBender::Mongo.client
      ...
      def process_message(message)
        ...
        state_object = RBender::StateHandler.new(message, @api, session, &state_block) # Object to invoke
      ...
      def on_state_changed(state_obj, state_old, state_new, message, session)
        if state_new != state_old # If state has changed
          ...
          state_obj = RBender::StateHandler.new(message, @api, session, &state_new_block)
